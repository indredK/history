...
8. **§1.6 / §2.8 单元测试持续投入** — 长期(本轮:前端 94 个 vitest 测试文件 861 个用例,
   ...
   全部 4 个函数,serviceFactory 的 createSimpleDataClient / createMultipleServices
   / ServiceMonitor 单例统计,apiClient(createApiClient 配置、normalizeAxiosError
   全分支 NETWORK/TIMEOUT/SERVER/CLIENT、401/403 → AUTH_REQUIRED_EVENT 派发、
   getApiStatus + fallbackControl 薄包装),apiTest 四个 fetch 包装器(连接 /
   端点 / 批量 / 代理)的成功/格式错误/HTTP错/抛错分支,religionStore
   (setHoveredNode 邻居计算、setSearchQuery 多字段模糊匹配、getFilteredNodes/Edges
   ...
   StorageListener 跨标签页 storage 事件灌入),scholarStore(in-house filter
   含 dynasty/dynastyPeriod 兜底 + 短路 OR + AND 复合),mythologyStore(category=null
   短路 / vi.mock filterByCategory 入参与返回透传),emperorStore + qingRulerStore
   (store filters → service.filterAndSort 入参重命名 searchQuery→query,
   dynasty/period options 静态计算),sanguoFigureStore(4 元 filters role/kingdom/
   ...
   小型组件 9 个(timeline 的 SearchBar focus 视觉反馈 + 受控 value 改变 +
   EventCard 渲染 / confidence 徽章 / onClick(event),mythology 的 MythologyCard
   极薄包装 + CategoryFilter chip + CategoryTabs mobile/desktop 通过 matchMedia
