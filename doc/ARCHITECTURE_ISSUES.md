...
| person | `GET/POST/PATCH/DELETE /persons`，service spec 存在 | 需要确认 source/person 关联写入、重复姓名处理、软删除/审计字段 |
| emperor | `GET /emperors`、`GET /emperors/:id`，service spec 存在 | 无写接口；与 dynasty/reign/eraName 的聚合和赛博帝王页数据源需统一 |
| figure/tang/song/yuan/ming/qing/sanguo | 每朝提供 legacy 和 canonical 列表、详情，service spec 存在；公共查询已补字符串校验，前端分朝代转换器已用 `unknown` 收口并白名单归一角色/阵营 | 只读；DTO 与各朝字段差异较多，新增朝代容易重复；时期字段、Yuan/Qing period 后端不生效等契约仍需文档化 |
| religion | `GET /religion/graph`、`GET /religions`、`GET /religions/:id`，service spec 存在 | 无节点/边 CRUD；大图查询缺少分页、缓存、布局预计算和权限控制 |
| culture | scholars 与 schools 均有 CRUD，service spec 存在；保存校验已覆盖空名称与学者生卒年 | scholar-school 双字段同步仍需跨 API/前端测试；PUT/PATCH 语义需统一 |
...
| people | 固定 tabs 覆盖档案、帝王、文化名人、唐宋元明清三国；档案 CRUD 可用 | 子模块数据加载刚修复，需补前端 Vitest；筛选枚举中文化和详情弹窗一致性继续复查 |
...
| 类型债 | `.github/workflows` 注释记录 frontend no-explicit-any、backend no-unsafe 历史问题；map service 和分朝代人物转换器已收口本轮发现的 `any`，其他模块仍需继续扫描 | lint 无法作为强 gate | P1 |
...
1. 已从 `dynasty` 模块补齐一轮 Controller/Service/DTO/测试模板，并在 `event` 模块补齐多标签筛选、CRUD、关联详情和前端事件管理入口；`person` 模块已补保存校验、表单错误态和 service spec mock 对齐；`emperor` 模块已补静态帝王过滤、nullable 在位结束、历史评价归一化和查询 DTO/spec 漂移；分朝代 `figure` 模块已补公共查询字符串校验和前端转换器类型收口/枚举归一；`religion` 模块已补主图 API 入口、兼容详情类型、period 校验和搜索高亮契约；`culture` 模块已补保存校验、弹窗错误态和详情空值展示；`mythology` 模块已补保存分类契约和表单错误清理；`map` 模块已补首屏边界、事件点空态、边界查询契约、地点查询过滤、预加载策略、tooltip 转义、资源路径兜底、service 类型收口、窄屏布局和图表错误恢复；公共前端链路已修复 dev proxy `/api/v1` 重复拼接、代理检测误用直连客户端和集合加载非数组崩溃风险；下一步继续补字段说明与前端测试。
