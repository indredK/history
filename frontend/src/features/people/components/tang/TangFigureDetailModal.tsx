...
export function TangFigureDetailModal({ figure, open, onClose }: TangFigureDetailModalProps) {
  ...
  const age = tangFigureService.calculateAge(figure);
  const roleLabel = ROLE_LABELS[figure.role];

  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth aria-labelledby="tang-figure-detail-title" slotProps={{ paper: { sx: { background: 'var(--color-bg-card)', borderRadius: 'var(--radius-lg)', maxHeight: '90vh' } } }}>
      <DialogTitle id="tang-figure-detail-title" sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', pb: 1 }}>
