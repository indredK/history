...
import { ROLE_LABELS, KINGDOM_LABELS, KINGDOM_COLORS } from '@/services/sanguoFigure/types';
...
export function SanguoFigureDetailModal({ figure, open, onClose }: SanguoFigureDetailModalProps) {
  ...
  const age = sanguoFigureService.calculateAge(figure);
        ...
        {figure.positions.length > 0 && (
          <Box sx={{ mb: 3 }}>
            <Typography variant="subtitle1" sx={{ color: 'var(--color-text-primary)', fontWeight: 600, mb: 1.5, display: 'flex', alignItems: 'center', gap: 1 }}>
              <WorkIcon sx={{ fontSize: '1.2rem', color: kingdomColor.text }} />担任职位
            </Typography>
        ...
        {figure.politicalViews && (
          <>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Box sx={{ p: 2, borderRadius: 'var(--radius-md)', backgroundColor: 'var(--color-bg-tertiary)', borderLeft: `3px solid ${kingdomColor.text}` }}>
                ...
                {figure.events.map((event, index) => (
                  <Box key={index} sx={{ p: 2, borderRadius: 'var(--radius-md)', backgroundColor: 'var(--color-bg-tertiary)', border: '1px solid var(--color-border)' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                      ...
                      <Chip label={event.role} size="small" variant="outlined" sx={{ fontSize: '0.7rem', height: '20px', borderColor: kingdomColor.text, color: kingdomColor.text }} />
                    </Box>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {figure.evaluations.map((evaluation, index) => (
                  <Box key={index} sx={{ p: 2, borderRadius: 'var(--radius-md)', backgroundColor: 'var(--color-bg-tertiary)', borderLeft: `3px solid ${kingdomColor.text}` }}>
