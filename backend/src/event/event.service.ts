...
import { TimelineResponseDto, TimelineEventDto } from './dto/timeline-event.dto';
...
export class EventService {
  ...
  async getTimeline(query: TimelineQueryDto): Promise<TimelineResponseDto> {
    ...
    const actualStartYear = events.length > 0 ? 
      Math.min(...events.map(e => e.startYear)) : 
      (startYear || 0);
    
    const actualEndYear = events.length > 0 ? 
