...
8. **§1.6 / §2.8 单元测试持续投入** — 长期(本轮:前端 23 个 vitest 测试文件 217 个用例,
   覆盖关键交互、store 集成、createFigureStore 工厂、errorHandling 全路径(降级 /
   重试 / 状态管理)、dataLoaders 全路径(fetch / cache / retry / API 响应解包)和
   PerformanceMonitor(mark/measure 含 falsy-start 已知行为锁定);
