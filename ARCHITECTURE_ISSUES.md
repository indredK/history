...
- 已配置 vitest,本轮已扩到 28 个测试文件 / 307 个用例
...
8. **§1.6 / §2.8 单元测试持续投入** — 长期(本轮:前端 28 个 vitest 测试文件 307 个用例,
   ...
   端点 / 批量 / 代理)的成功/格式错误/HTTP错/抛错分支,以及 religionStore
   (setHoveredNode 邻居计算、setSearchQuery 多字段模糊匹配、getFilteredNodes/Edges
   多维筛选、getRelatedNodesAndEdges、resetFilters);后端 `DynastyService` / `PersonService` / `EventService` /
